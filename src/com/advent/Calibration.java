...
public class Calibration {
    private static final String data = "2911threeninesdvxvheightwobm\n" +
    ...
    private static final String[] DIGITS_REVERSED = Arrays.stream(DIGITS).map(digit -> new StringBuilder(digit).reverse().toString()).toArray(String[]::new);

    public static void main(String[] args) {
        String[] lines = data.split("\\n");
        int[] ints = Arrays.stream(lines).mapToInt(Calibration::mapToNumber).toArray();
        int sum = Arrays.stream(ints).sum();
        System.out.println(sum);
    }

    private static int mapToNumber(String line) {
        return Integer.parseInt(String.valueOf(findFirst(line)) + findLast(line));
    }

    private static String findFirst(String line) {
        int idx = 0;
        while (!isDigit(line.charAt(idx))) {
            String subLine = line.substring(idx);
            for (int i = 0; i < DIGITS.length; i++) {
                if (subLine.startsWith(DIGITS[i])) {
                    return String.valueOf(i + 1);
                }
            }
            idx++;
        }
        return String.valueOf(line.charAt(idx));
    }

    private static String findLast(String line) {
        String lineReversed = new StringBuilder(line).reverse().toString();
        int idx = 0;
        while (!isDigit(lineReversed.charAt(idx))) {
            String subLine = lineReversed.substring(idx);
            for (int i = 0; i < DIGITS_REVERSED.length; i++) {
                if (subLine.startsWith(DIGITS_REVERSED[i])) {
                    return String.valueOf(i + 1);
                }
            }
            idx++;
        }
        return String.valueOf(lineReversed.charAt(idx));
    }
